...
COMMAND_HANDLER(handle_jtag_select_fpga_chip_command)
{
	struct jtag_command *jtag_cmd = cmd_queue_alloc(sizeof(struct jtag_command));
	jtag_queue_command(jtag_cmd);
	jtag_cmd->type = JTAG_SELECT_CHIP;
	jtag_execute_queue();
	return jtag_init(CMD_CTX);
}

COMMAND_HANDLER(handle_jtag_select_softcore_command)
{
	struct jtag_command *jtag_cmd = cmd_queue_alloc(sizeof(struct jtag_command));
	jtag_queue_command(jtag_cmd);
	jtag_cmd->type = JTAG_SELECT_SOFTCORE;
	jtag_execute_queue();
	return jtag_init(CMD_CTX);
}
...
static const struct command_registration jtag_subcommand_handlers[] = {
	...
	{
		.name = "select_fpga_chip",
		.mode = COMMAND_ANY,
		.handler = handle_jtag_select_fpga_chip_command,
		.help = "attach jtag interface to FPGA chip TAP",
		.usage = ""
	},
	{
		.name = "select_softcore",
		.mode = COMMAND_ANY,
		.handler = handle_jtag_select_softcore_command,
		.help = "attach jtag interface to softcore TAP",
		.usage = ""
	},
	{
		.name = "arp_init",
